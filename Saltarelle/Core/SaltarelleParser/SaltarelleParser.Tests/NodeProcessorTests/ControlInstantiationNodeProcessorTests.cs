...
namespace SaltarelleParser.Tests {
	...
	public class ControlInstantiationNodeProcessorTests : NodeProcessorTestBase {
		...
		public void TestTryProcess_AttributesExceptCustomInstantiateMapCorrectly() {
			...
			Expect.Call(() => template.AddMember(new InstantiatedControlMember("TestId", "Namespace.TestType", false, new Dictionary<string, TypedMarkupData>() { { "Attr1", new TypedMarkupData("value1") }, { "Attr2", new TypedMarkupData("value2") } }, false, new IMember[0])));
			...
			Assert.AreEqual("[CONTROL id=TestId inner=no]", ConcatenatedFragments);
			mocks.VerifyAll();
		}
		...
		public void TestTryProcess_CustomInstantiateMapCorrectly() {
			Expect.Call(template.HasMember("TestId")).Return(false);
			Expect.Call(() => template.AddMember(new InstantiatedControlMember("TestId", "Namespace.TestType", true, new Dictionary<string, TypedMarkupData>(), false, new IMember[0])));
			Expect.Call(() => renderFunction.AddDependency(null)).IgnoreArguments().Constraints(Property.Value("Name", "TestId"));
			mocks.ReplayAll();
		...
		public void TestTryProcess_MissingIdCausesDefault() {
			...
			Expect.Call(() => template.AddMember(new InstantiatedControlMember("_test_id", "Namespace.TestType", false, new Dictionary<string, TypedMarkupData>(), false, new IMember[0])));
			...
			Assert.AreEqual("[CONTROL id=_test_id inner=no]", ConcatenatedFragments);
			mocks.VerifyAll();
		}
		...
		public void TestTryProcess_ChildrenWorks() {
			XmlNode node = Globals.GetXmlNode("<control id=\"TestId\" type=\"TestType\"><elem1/>Text node<elem2><elem3/></elem2></control>");
			XmlNode n1 = node.SelectSingleNode("elem1");
			XmlNode n2 = n1.NextSibling;
			XmlNode n3 = n2.NextSibling;
			...
			using (mocks.Ordered()) {
				...
				Expect.Call(template.HasMember("TestId_inner")).Return(false);
				Expect.Call(() => template.AddMember(CreateRenderFunction("TestId_inner", "", new[] { new LiteralFragment("[n1]"), new LiteralFragment("[n2]"), new LiteralFragment("[n3]") })));
				...
				Expect.Call(() => template.AddMember(null)).IgnoreArguments().Constraints(Is.Matching((InstantiatedControlMember m) =>
				                                                                                      ...
				                                                                                      m.HasInnerHtml &&
				                                                                                      m.Dependencies.SequenceEqual(new[] { "TestId_inner" })));
			}

			mocks.ReplayAll();

			Assert.IsTrue(new ControlInstantiationNodeProcessor().TryProcess(docProcessor, node, false, template, renderFunction));
			Assert.AreEqual("[CONTROL id=TestId inner=yes]", ConcatenatedFragments);

			mocks.VerifyAll();
		}
		
		[TestMethod]
		public void TestTryProcess_NoFragmentsForInnerElementWorks() {
			XmlNode node = Globals.GetXmlNode("<control id=\"TestId\" type=\"TestType\"><elem/></control>");
			XmlNode n1 = node.SelectSingleNode("elem");
			
			Expect.Call(() => renderFunction.AddDependency(null)).IgnoreArguments().Constraints(Property.Value("Name", "TestId"));
			Expect.Call(() => docProcessor.ProcessRecursive(null, null, null)).Do((Action<XmlNode, ITemplate, IRenderFunction>)((n, tpl, _) => { Assert.AreEqual(n, n1); Assert.AreEqual(tpl, template); })).IgnoreArguments();
			Expect.Call(template.HasMember("TestId")).Return(false);
			Expect.Call(() => template.AddMember(new InstantiatedControlMember("TestId", "TestType", false, new Dictionary<string, TypedMarkupData>(), false, new IMember[0])));
