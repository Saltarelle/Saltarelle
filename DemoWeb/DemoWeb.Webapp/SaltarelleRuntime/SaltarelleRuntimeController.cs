...
namespace DemoWeb.Webapp.Controllers {
	...
	public class SaltarelleRuntimeController : Controller {
		public ActionResult AssemblyScript(string assemblyName) {
			return JavaScript(GlobalServices.GetService<IScriptManagerService>().GetAssemblyScriptContent(AppDomain.CurrentDomain.GetAssemblies().Where(asm => asm.GetName().Name == assemblyName).SingleOrDefault()));
		}
	}
}
